...
"""
Test Module for Open Document (.odt) Introduction example

This module contains tests to ensure that the Open Document files generated by
the `example_msl-odt_introduction.py` script match the expected output file,
`example_msl-odt_introduction.odt`, located in `expected_outputs` directory.

The tests focus on the following aspects:
- The correct creation of `example_msl-odt_introduction.odt` by running the
  `example_msl-odt_introduction.py` script.
...
`example_msl-odt_introduction.py` script before any tests are executed.
This fixture:
- Runs the script to create `example_msl-odt_introduction.odt`
  in the same directory as the script.
- Ensures the file has been created successfully.
- Cleans up by deleting the generated `example_msl-odt_introduction.odt`
...
SCRIPT_FILE = SCRIPT_DIR / "example_msl-odt_introduction.py"
# Test file created
TEST_FILE = SCRIPT_DIR / "example_msl-odt_introduction.odt"
# Expected output file
EXPECTED_FILE = EXPECTED_OUTPUTS_DIR / "example_msl-odt_introduction.odt"

# Run the script to create example_msl-odt_introduction.odt at the start
...
def test_text_elements_exist():
    ...
    if not EXPECTED_FILE.exists():
        pytest.skip(f"Skipping {test}: file not found.")
...
def test_text_content_equality():
    ...
    if not EXPECTED_FILE.exists():
        pytest.skip(
            "Skipping test_text_content_equality: "
...
def test_metadata_creator():
    if not TEST_FILE.exists():
        pytest.skip("Skipping test_metadata_creator: test file not found.")
    if not EXPECTED_FILE.exists():
        pytest.skip("Skipping test_metadata_creator: golden file not found.")
    # Load both documents
    doc1 = load(TEST_FILE)
    doc2 = load(EXPECTED_FILE)
...
def test_metadata_title():
    if not TEST_FILE.exists():
        pytest.skip("Skipping test_metadata_title: test file not found.")
    if not EXPECTED_FILE.exists():
        pytest.skip("Skipping test_metadata_title: golden file not found.")
    # Load both documents
    doc1 = load(TEST_FILE)
    doc2 = load(EXPECTED_FILE)
